...
/*****************NodePtr***************************/
#[derive(Debug)]
pub struct NodePtr<K: Ord, V>(pub(crate) *mut RBTreeNode<K, V>);
...
impl<K: Ord, V> RBTree<K, V> {
    ...
    #[inline]
    pub fn insert(&mut self, k: K, v: V) -> NodePtr<K,V>{
        self.len += 1;
    ...
    #[inline]
    pub fn get_first_ptr(&self) -> (Option<(&K, &V)>, NodePtr<K, V>) {
        let first = self.first_child();
        if first.is_null() {
            return (None, NodePtr::null());
        }
        unsafe { (Some((&(*first.0).key, &(*first.0).value)), first) }
    }
    ...
    #[inline]
    pub fn get_first_mut_ptr(&mut self) -> (Option<(&K, &mut V)>, NodePtr<K, V>) {
        let first = self.first_child();
        if first.is_null() {
            return (None, NodePtr::null());
        }
        unsafe { (Some((&(*first.0).key, &mut (*first.0).value)), first) }
    }
    ...
    #[inline]
    pub unsafe fn delete(&mut self, node: NodePtr<K, V>) -> (K, V) {
        let mut child;
        let mut parent;
        let color;
